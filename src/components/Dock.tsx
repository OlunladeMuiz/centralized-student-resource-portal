    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={`${className} absolute -top-8 left-1/2 w-fit whitespace-pre rounded-lg border-3 border-[#0F172A] bg-[#FAFAF9] px-3 py-1.5 text-[#0F172A] shadow-[4px_4px_0px_0px_rgba(15,23,42,1)]`}
          role="tooltip"
          style={{ x: '-50%', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}
        >
          {children}
        </motion.div>
      )}
  ...
  return (
    <motion.div style={{ height, scrollbarWidth: 'none' }} className="mx-2 flex max-w-full items-center">
      <motion.div
        ...
        className={`${className} absolute bottom-2 left-1/2 transform -translate-x-1/2 flex items-end w-fit gap-4 rounded-2xl border-[#0F172A] border-4 pb-3 px-4 bg-[#FAFAF9]/95 backdrop-blur-sm shadow-[8px_8px_0px_0px_rgba(15,23,42,1)]`}
