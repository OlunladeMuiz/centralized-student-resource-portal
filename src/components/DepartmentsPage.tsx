...
export function DepartmentsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <h1 className="mb-3">Department Directory</h1>
        <p className="opacity-70">Find contact information and services for all campus departments</p>
      </motion.div>
        ...
        <div className="bg-card border-4 border-primary p-6 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)]">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-12 h-14 border-3 border-primary bg-input-background focus:border-accent focus:shadow-[4px_4px_0px_0px_rgba(6,255,165,1)] transition-all"
            />
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {filteredDepartments.map((dept, index) => {
          ...
          >
            <SpotlightCard 
              className={`p-0 ${scheme.shadow} ${scheme.hover} hover:translate-y-[-4px] transition-all h-full`}
              spotlightColor="rgba(245, 158, 11, 0.2)"
            >
              <div className={`${scheme.bg} border-b-4 ${scheme.border} p-6`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <h3 className="mb-2 text-white">{dept.name}</h3>
                    <p className="text-white/90" style={{ fontSize: '0.9375rem', fontFamily: 'var(--font-body)' }}>{dept.description}</p>
                  </div>
                  <span className={`px-3 py-1 bg-white ${scheme.border} border-3 text-[#0F172A]`} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase' }}>
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    <Mail className="w-5 h-5 text-[#F59E0B] mt-1 flex-shrink-0" />
                    <a href={`mailto:${dept.contact}`} className="text-[#F59E0B] hover:underline" style={{ fontFamily: 'var(--font-mono)' }}>
                      {dept.contact}
                    </a>
                  </div>
                  <div className="flex items-center gap-3">
                    <Phone className="w-5 h-5 text-[#64748B] flex-shrink-0" />
                    <span style={{ fontFamily: 'var(--font-mono)' }}>{dept.phone}</span>
                  </div>
                  <div className="flex items-start gap-3">
                    <MapPin className="w-5 h-5 text-[#64748B] mt-1 flex-shrink-0" />
                ...
                {/* Services */}
                <div className="pt-3">
                  <h4 className="mb-3" style={{ textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}>Services Offered</h4>
                  <div className="flex flex-wrap gap-2">
                    {dept.services.map(service => (
                      <span 
                        key={service} 
                        className="px-3 py-1 bg-[#FEF3C7] border-2 border-[#F59E0B] text-[#0F172A]"
